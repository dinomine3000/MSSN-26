...
public class ConditionalBrakeFood extends Behaviour {

	public ConditionalBrakeFood(float weight) {
		super(weight);
	}
	...
	private static int getFoodCount(List<Body> bodies) {
		int i = 0;
		for(Body b: bodies) {
			if(b instanceof CellBodyProxy cell && cell.getPatch().getState() == WorldConstants.PatchType.FOOD.ordinal()) i++;
		}
		return i;
	}
